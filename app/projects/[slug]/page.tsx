  ...
  return (
    <main className="container-shell py-12 sm:py-16">
      ...
      <article className="mt-12">
        ...
        {project.slug === 'ai-agents-simplified' && <NewsletterGrowth />}
...
function NewsletterGrowth() {
  return <section className="mt-20 border-t border-[var(--line)] pt-12 sm:mt-24 sm:pt-16">
    <div className="grid gap-6 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><div><p className="eyebrow">Newsletter growth & monetisation</p><h2 className="mt-4 text-4xl font-semibold leading-[.98] tracking-[-.06em] text-[var(--ink)] sm:text-5xl">Treating the newsletter as a product — not just a publishing channel.</h2></div><p className="max-w-xl text-base leading-7 text-[var(--ink-muted)]">Audience signals, content performance, competitor patterns, and monetisation research informed a sequence of focused experiments across format, distribution, links, and revenue.</p></div>
    <div className="mt-10 grid gap-5 lg:grid-cols-[1.08fr_.92fr] lg:items-stretch"><figure className="overflow-hidden rounded-[1.4rem] border border-[#2b302e] bg-[#151717] p-2 shadow-soft sm:p-3"><Image src="/images/ai-agents-simplified/newsletter-growth-dashboard.png" alt="AI Agents Simplified subscriber, revenue, and views growth dashboard" width={1329} height={828} className="h-auto w-full rounded-xl" /><figcaption className="px-2 pb-1 pt-4 text-xs leading-5 text-white/60">Reported newsletter dashboard snapshot: subscriber growth, pledged annualized revenue, and one-year views.</figcaption></figure><div className="grid gap-px overflow-hidden rounded-[1.4rem] border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2 lg:grid-cols-1"><GrowthDetail number="01" title="Audience & format experiments" text="Survey feedback, post impressions, and high-performing competitors shaped planned tests of newsletter structures and audience content." /><GrowthDetail number="02" title="Engagement balance" text="The resulting mix helped broader educational content move from roughly 20–22 to 60–70 likes and comments." /><GrowthDetail number="03" title="CTR & SEO research" text="Testing showed that more relevant internal and external links could improve click-through performance." /><GrowthDetail number="04" title="Revenue experiments" text="CPC advertising and CEO interview concepts doubled revenue during weaker periods; paid newsletter content is now being prepared." /></div></div>
    <div className="mt-5 grid gap-3 sm:grid-cols-3"><MetricCard value="57,366" label="Subscribers" detail="from 31,231 · +83.7%" /><MetricCard value="$652" label="Pledged annualized revenue" detail="from $96 · +579.2%" /><MetricCard value="1.18M" label="One-year views" detail="from 133K · +789.5%" /></div>
  </section>;
}

function GrowthDetail({ number, title, text }: { number: string; title: string; text: string }) {
  return <div className="bg-[var(--card)] p-5"><p className="text-sm font-bold text-[#76961d]">{number}</p><h3 className="mt-4 text-lg font-semibold tracking-[-.035em] text-[var(--ink)]">{title}</h3><p className="mt-2 text-sm leading-6 text-[var(--ink-muted)]">{text}</p></div>;
}

function MetricCard({ value, label, detail }: { value: string; label: string; detail: string }) {
  return <div className="rounded-xl border border-[var(--line)] bg-[var(--accent-soft)] p-5"><p className="text-3xl font-semibold tracking-[-.06em] text-[var(--ink)]">{value}</p><p className="mt-2 text-sm font-bold text-[var(--ink)]">{label}</p><p className="mt-1 text-xs text-[var(--ink-muted)]">{detail}</p></div>;
}

function AiAgentsAutomation() {
  return <section className="mt-20 border-t border-[var(--line)] pt-12 sm:mt-24 sm:pt-16">
    <div className="grid gap-6 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><div><p className="eyebrow">Growth operations</p><h2 className="mt-4 text-4xl font-semibold leading-[.98] tracking-[-.06em] text-[var(--ink)] sm:text-5xl">Reply-aware, personalized outreach at campaign scale.</h2></div><p className="max-w-xl text-base leading-7 text-[var(--ink-muted)]">I designed this n8n workflow to turn an Apollo list of roughly 6,000 prospects into controlled, personalized outreach without continuing to message people who had already responded.</p></div>
    ...
    <div className="mt-4 grid gap-px overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--line)] md:grid-cols-2 xl:grid-cols-4"><AutomationDetail number="01" title="Personalized sending" text="Apollo contacts were prepared for tailored messages, distributed across four to five sender inboxes." /><AutomationDetail number="02" title="Structured follow-up" text="Campaign batches of 20 contacts received two sequenced follow-ups after three and four days." /><AutomationDetail number="03" title="Reply-aware stop rule" text="Gmail reply checks and contact-state updates prevented follow-ups after a recipient replied." /><AutomationDetail number="04" title="Cost & event impact" text="The workflow avoided an estimated $50–80 monthly tooling cost and supported the sell-out of 60 first-event tickets." /></div>
